...
#x_train_temp, y_train_temp = mk_dataset.mk_dataset()
x_train_temp, y_train_temp = extract_training_samples("balanced")
...
##パラメーター設定
NEURONS = 128									#ニューロン数
ACTIVATION = 'softplus'							#活性化関数
OPTIMIZER = optimizers.Adamax()					#最適化アルゴリズム
LOSS = 'sparse_categorical_crossentropy'		#損失関数
BATCH = 5000									#バッチサイズ
RATIO = 0.2										#Dropout層の割合
INITIALIZER = 'RandomNormal'					#重みの初期化
L2 = 0.0000975									#Weight decayのL2ノルム
file_name = 'test_data.txt'				#historyを保存するファイル名
row = 3											#ファイル内の何番目を参照するか
title = 'test_rote+move+scale_1'						#グラフの名前
subtitle = 'test_rote+move+scale_1'					#グラフ内のタイトル
...
model = models.Sequential([
  layers.Conv2D(NEURONS,(3,3),activation=ACTIVATION,padding=PADDING,strides=STRIDES,kernel_initializer=INITIALIZER,input_shape=(28,28,1)),
  layers.Conv2D(NEURONS,(3,3),activation=ACTIVATION,padding=PADDING,strides=STRIDES,kernel_initializer=INITIALIZER),
  layers.MaxPooling2D(pool_size=(2,2)),
  layers.Conv2D(NEURONS,(3,3),activation=ACTIVATION,padding=PADDING,strides=STRIDES,kernel_initializer=INITIALIZER),
  layers.GlobalMaxPooling2D(),
  layers.Flatten(),
  layers.Dropout(RATIO),
  layers.Dense(NEURONS,activation = ACTIVATION,kernel_initializer=INITIALIZER),
  layers.Dropout(RATIO),
  layers.Dense(NEURONS,activation = ACTIVATION,kernel_initializer=INITIALIZER),
  layers.Dropout(RATIO),
  layers.Dense(NEURONS,activation = ACTIVATION,kernel_initializer=INITIALIZER),
...
##実行
history = model.fit(x_train, y_train,batch_size = BATCH,validation_split=0.2,epochs=10)
